...
pub(crate) struct ToplevelsInfo {
    pub title: String,
    pub app_id: String,
}

pub(crate) fn refresh_toplevels() -> Vec<ToplevelsInfo> {
    ...
    for toplevel in app_data.toplevels.iter() {
        toplevels.push(ToplevelsInfo {
            title: toplevel
                .title
                .clone()
                .unwrap_or_else(|| "Unnamed".to_string()),
            app_id: toplevel.app_id.clone().unwrap_or_else(|| "".to_string()),
        });
    }
